...
#    1. docker build  (from java-streaming/Dockerfile)
#    2. docker tag
#    3. docker push   (authenticated via provider block in versions.tf)
#
#  triggers.dockerfile_hash forces a rebuild+push whenever
#  the Dockerfile or pom.xml changes. To force a rebuild after
#  editing Java sources, touch pom.xml or run `tofu apply -replace=docker_image.app`.
# ─────────────────────────────────────────────
...
resource "docker_image" "app" {
  ...
  # Rebuild when Dockerfile or Maven build descriptor changes
  triggers = {
    dockerfile = filesha256("${path.module}/../Dockerfile")
    pom_xml    = filesha256("${path.module}/../pom.xml")
  }
