...
resource "aws_api_gateway_rest_api" "api" {
  ...
  description = "Streaming REST API — Spring Boot SSE + Lambda Web Adapter"
...
resource "aws_api_gateway_method" "get_stream" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.stream.id
  http_method   = "GET"
  authorization = "NONE"

  # Pass query string params (?ai=true&seed=42) through to Lambda
  request_parameters = {
    "method.request.querystring.ai"   = false
    "method.request.querystring.seed" = false
  }
}
...
resource "aws_api_gateway_integration" "stream_lambda" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.stream.id
  http_method             = aws_api_gateway_method.get_stream.http_method
...
# ─────────────────────────────────────────────
#  CORS preflight: OPTIONS /search/stream
# ─────────────────────────────────────────────
...
resource "aws_api_gateway_deployment" "deploy" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.get_stream.id,
